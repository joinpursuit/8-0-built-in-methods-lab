...
function getPercentageScore(score) {
  let percentage = Math.round((score.received / score.max) * 100)
  return percentage
}
...
function getHighestOfTwoScores(score1, score2) {
  let firstPercentage = Math.round((score1.received / score1.max) * 100);
  let secondPercentage = Math.round((score2.received / score2.max) * 100);
  let totalPercent = Math.max(firstPercentage,secondPercentage);
  return totalPercent;
}
